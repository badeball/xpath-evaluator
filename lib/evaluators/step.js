...
module.exports = {
  evaluate: function (step, context, type) {
    ...
    if (step.test.type && step.test.type !== XPathParser.NodeType.NODE) {
      var nodeType;

      switch (step.test.type) {
        case XPathParser.NodeType.COMMENT:
          nodeType = Node.COMMENT_NODE;
          break;

        case XPathParser.NodeType.PROCESSING_INSTRUCTION:
          nodeType = Node.PROCESSING_INSTRUCTION_NODE;
          break;

        case XPathParser.NodeType.TEXT:
    ...
    if (step.predicates) {
      var reversed = (
          step.axis === XPathParser.AxisSpecifier.ANCESTOR ||
          step.axis === XPathParser.AxisSpecifier.ANCESTOR_OR_SELF ||
          step.axis === XPathParser.AxisSpecifier.PRECEDING ||
          step.axis === XPathParser.AxisSpecifier.PRECEDING_SIBLING);
